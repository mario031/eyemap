...
//  EyeMap
//
//  Created by mario on 2016/07/29.
...
class DateManager: NSObject {
    ...
    //月の最終日を取得
    func lastDateOfMonth() -> Int {
        let calendar:NSCalendar = NSCalendar.currentCalendar()
        let range:NSRange = calendar.rangeOfUnit(NSCalendarUnit.Day, inUnit: NSCalendarUnit.Month, forDate: selectedDate)
        let dayCount:NSInteger = range.length
        let lastDate:Int = dayCount + self.firstDate() - 1
        return lastDate
    }
